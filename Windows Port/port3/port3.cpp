...
//		CreateWindowEx(WS_EX_CLIENTEDGE, TEXT("Textbox"), TEXT("Text here 12345"), WS_VISIBLE | WS_CHILDWINDOW | WS_BORDER | ES_AUTOHSCROLL, 100, 30, 300, 20, hwnd, (HMENU)ID_EDITTEXT, NULL, NULL);
	...
	case WM_APP+2:
		if(n_que_temp - 1)
		{
			...
			data[count - 1].setDatabase(ques, a1, a2, a3, a4, c_ans);
		...
		{	
			...
			/*itoa( data[count-1].getCorrect(), a4, 10);
			MultiByteToWideChar(CP_ACP, 0, a4, strlen(a4)+1, str, strlen(a4)+1);
			MessageBox(hwnd, str, L"Test", MB_OK);*/
	...
	if(!f1)
	{
		MessageBox(hwnd, L"could not open stud.dat file for output!", L"fatal error", MB_OK);
		exit(0);	
	}
	f1 << "<html>";
	f1 << endl;
